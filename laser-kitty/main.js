...
import { CatRig } from './catrig.js?v=k52';
import { Career } from './career.js?v=k52';
import { mountNav } from './nav.js?v=k52';
...
const wasm = await WebAssembly.instantiateStreaming(fetch('lk_core.wasm?v=k52'), {});
...
function applyLightBudget() {
  ...
  for (const pk of picks) {
    ...
    const wp = pk.node.position;
    const hl = Math.hypot(wp.x, wp.z);
    if (hl > 0.4) pl.position.set(-(wp.x / hl) * 0.28, 0.06, -(wp.z / hl) * 0.28);
    if (wp.y > 2.2) pl.position.y = -0.35; // ceiling fixtures light from below themselves
    pk.node.add(pl);
    litLights.push(pl);
  }
}
...
function meshFor(i, shape, a, b, c, cls, py, gloss, tint, px) {
  ...
  if (cls === 2 && shape === 1 && a < 0.011 && gloss > 0.985) {
    ...
    // paw prints (0.74) are SEPARATE reserved gloss signatures — prints
    // are a derivative system sourced from the puddle sim, and the
    // viewer never infers which is which from dims (a puddle spawns
    // print-sized and floods afterward, so any dims key eventually lies
    ...
    m.add(glow);
    registerLight(m, 0xffc98a, 1.5, 2.8);
    ...
    m.userData.noShadow = true;
  } else if (cls === 0 && shape === 0 && Math.abs(gloss - 0.86) < 0.005) {
    ...
    m.add(core);
    registerLight(m, 0xffd9a0, 2.4, 3.8);
    m.userData.noShadow = true;
    ...
    m.add(core2);
    registerLight(m, 0xffd9a0, 1.4, 3);
  ...
  while (acc >= DT) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (ev === 3 || ev === 4) {
        ...
        // Small pops, no shake, no puffs — the prop IS the visual.
        const kind = (code >> 25) & 0x7;
        popScore(kind === 1 ? `sparks +${code & 0xfff}` : `+${code & 0xfff}`);
      }
    }
    acc -= DT;
  }
      ...
      if (cfg.catver !== 'v1') {
        ...
        if (view.rig) {
          ...
          view.rig.update(frameDt / 1000, {
            ...
            // real vertical motion = the sim owns the height; the rig's
            // measured-contact pass must not fight a jump or a fall
            airborne: st === 4 || Math.abs(vy) > 0.8,
          });
        }
      } else if (view.rig) {
...
window.__lk = {
  ...
  meshAt: (i) => {
    const m = meshes[i];
    return m ? { puddle: !!m.userData.puddle, scale: +m.scale.x.toFixed(3), kids: m.children.length, vis: m.visible, y: +m.position.y.toFixed(4) } : null;
  },
  screenAt: (i) => {
    const m = meshes[i];
    if (!m) return null;
